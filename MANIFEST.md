...
本清单完整记录 `final_submission_package/` 归档包中全部有效文件及其与正文论文章节、图表的严格对应关系。
...
| 磁盘文件路径 | 文件类型 | 说明与用途 |
|---|---|---|
| `流行病传播动力学可预测视界_最终稿.pdf` | PDF 文档 | 唯一官方权威终稿：基于最新 `paper_cn_journal_template/main.tex` 编译生成之完整期刊论文（42 页，与 `paper_cn_journal_template/main.pdf` 字节哈希完全相同） |
...
| `paper_cn_journal_template/main.pdf` | Tectonic 编译官方输出 (1.36 MiB，与根目录最终稿哈希完全一致) |
| `paper_cn_journal_template/fig1_framework.png` | 理论原理与双层基准框架图高清矢量渲染图 |
...
## 8. 分析中间结果与报告数据 (`reports/` & `reports_v3/`)
| 磁盘文件路径 | 说明 |
|---|---|
