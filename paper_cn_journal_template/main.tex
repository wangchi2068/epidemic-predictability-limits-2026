...
\end{enumerate}
目标容忍度设为 $\tau = 0.5$（对应相对均方根误差 50\%），采用 2,000 次参数化 Bootstrap 重抽样（固定随机种子 42）评估动力学参数与视界置信区间。
...
\end{equation}
根据假定，外生环境扰动序列 $\{\eta_t\}$ 在代际间独立同分布，其累计和的方差为 $\text{Var}(\sum_{t=1}^h \eta_t) = h v^2$。对于微观负二项抽样扰动序列 $\{\xi_t\}$，条件于前一步的历史发病轨迹，由于传播事件满足正交鞅差性质，更新创新项之间互不相关。根据对数函数的一阶 Delta 展开，单代相对抽样方差为 $\text{Var}(\xi_t \mid \mathcal{F}_{t-1}) \approx 1/\mathbb{E}[Z_t] + 1/k$。在宏观大样本发病假定（$\mathbb{E}[Z_t] \gg 1$）下，泊松离散项 $1/\mathbb{E}[Z_t] \to 0$，单代方差饱和于过度离散倒数常数 $1/k$。对 $h$ 个独立步长实施方差累加，即有 $\text{Var}(\sum_{t=1}^h \xi_t) \approx h/k$。对于发病归零的边界情形，引入标准的正则化平滑变量 $\log(Z_h + 1)$，其与原对数序列的偏差阶数为 $O(1/Z_h)$，在非灭绝轨道上数学严格良定。将两部分独立方差线性相加，即严格证得定理结论。 在后文第 4 节的实证记账中，传播率时变漂移项以一阶差分经验方差 $\mathcal{E}_{\text{drift}}(h) = \frac{h \cdot \widehat{\text{Var}}(\Delta \log R_t)}{R^2}$ 作为经验代理量。在随机游走假设下，一阶差分方差满足 $\text{Var}(\Delta \log R_t) \approx 2 v^2$，因子 $1/R^2$ 系通过 Delta 方法将对数尺度方差映射至相对发病方差尺度，而定理 5 中的宏观抽样项 $1/k$ 已在记账中由微观分支变异系数 $\text{CV}^2$ 或未归因闭合余项吸纳，避免跨尺度重复计入。
证毕。
\end{proof}
...
\begin{itemize}
\item \textbf{多层感知机（MLP）神经估计器参数估计}：在符合负二项分支似然的生成数据上训练 MLP 神经估计器（架构包含 2 个隐藏层，每层 64 个神经元，ReLU 激活函数，Adam 优化器学习率 $\eta=10^{-3}$，早停耐心轮数 15，在 10,000 条合成轨迹窗口上按 8:2 划分训练与验证集，固定随机种子 42）。基准测试显示，所测试的浅层 MLP 神经估计器中位经验方差与 CRB 下界之比为 6.69（反映小样本浅层网络在离散分支噪声下的统计效率损耗，而非全类机器学习算法的固有上界），而经典有效极大似然估计量在统计不确定度内与 CRB 下界严格吻合（中位比值 1.000，95\% 置信区间为 0.995--1.002）。这表明参数神经网络的灵活拟合能力并不能突破底层离散观测噪声所施加的信息论 Cram\'er--Rao 方差下界约束。
\item \textbf{LightGBM 梯度提升树多步预测}：基于 200 条完整合成疫情序列（每条涵盖 52 周前瞻点），在 5 折交叉验证与网格搜索调优下（配置 100 棵回归树，最大树深 5，学习率 0.05，叶节点最小样本数 10，固定随机种子 42）评估 LightGBM 的前瞻对数发病预测性能。结果显示，所实现的 LightGBM 运行在理论条件标准差下界之上约 1\%--18\%（在有限样本与非耗尽超参搜索下，无偏预测器无法穿透群体随机性方差下界）。
\end{itemize}
...
\midrule
COVID-19 Delta 暴发期 & 1.3149 & 0.0285 & 223.2 & 28,347 & 4.7 & 23.1 / 15.5 & 20.1 / 13.5 (8.9--20.0) & -- \\
COVID-19 Omicron 达峰期 & 1.1632 & 0.0762 & 27.4 & 66,275 & 3.0 & 7.6 / 3.3 & 6.8 / 2.9 (1.8--6.0) & -- \\
COVID-19 JN.1 流行期 & 1.0520 & 0.0557 & 60.5 & 31,453 & 3.5 & 9.4 / 4.7 & 8.4 / 4.2 (2.6--8.5) & -- \\
RSV 2024--25 流行季 & 1.2404 & 0.0146 & 1000.0 & 4,409 & 8.4 & 42.4 / 50.9 & 36.6 / 43.9 (27.9--65.0) & 16--20 周截断 \\
RSV 2025--26 流行季 & 1.2130 & 0.0089 & 1000.0 & 1,868 & 8.4 & 67.8 / 81.4 & 58.4 / 70.1 (46.2--95.0) & 16--20 周截断 \\
流感 2022--23 暴发早期 & 1.4138 & 0.0557 & 50.2 & 3,116 & 3.2 & 12.7 / 5.8 & 11.2 / 5.1 (3.3--10.2) & -- \\
流感 2024--25 流行季 & 1.4012 & 0.0388 & 111.7 & 7,586 & 3.2 & 18.0 / 8.2 & 15.8 / 7.2 (4.8--14.0) & -- \\
\bottomrule
\end{tabular}%
}
\vspace{2pt}
\begin{flushleft}
\scriptsize \textbf{注：}为保证与表~\ref{tab:budget_four} 完全推导对齐，$\delta R$ 统一保留小数点后 4 位。代间隔 $\mu_g$ 取自权威文献共识与 CDC 生产设定：Delta 取内在代间隔 4.7 天\citep{hart2022generation}；Omicron BA.1 采用 Park 等\citep{park2023inferring} 基于美国家户前向追踪测得的实现代间隔（realized generation interval）3.0 天（95\% 置信区间 2.7--3.2 天，灵敏度分析表明若按 Hart 等\citep{hart2022generation} 测算的内在代间隔约 3.4 天折算，周视界由 2.9 周微调至 3.3 周，核心紧凑特征不变）；JN.1 采用 Chan 等\citep{chan2026estimating} 基于美国家户数据测得的同源 XBB 亚分支内在代间隔 3.5 天（95\% CrI 3.1--3.9 天）；流感两季取内在代间隔 3.2 天\citep{chan2025influenza}；RSV 取南非家户前向实测代间隔基准 8.4 天\citep{cohen2024respiratory}（幼儿日托高频接触敏感性对照为 3.2 天）。$h^*_{\text{approx}}$ 为一阶闭式公式 (\ref{eq:horizon_cn}) 导出的领先阶渐近理论视界；$h^*_{\text{exact}}$ 为满足全方差临界方程（$\tau=0.5$）的数值积分根；周数按 $h^*_{\text{周}} = h^* \cdot (\mu_g / 7)$ 折算；95\% 置信区间基于 2,000 次参数化 Bootstrap 重抽样。RSV 两季理论外推根（43.9 与 70.1 周）受单流行季自然物理跨度（约 16--20 周）外部截断约束。
\end{flushleft}
\end{table}
...
\vspace{2pt}
\begin{flushleft}
\scriptsize \textbf{注：}各分量绝对值按 $10^{-4}$ 计量，括号内为占该行实测总相对均方误差的百分比；各行前瞻步长严格按 $h_{\text{gen}} = h_{\text{周}} \cdot 7 / \mu_g$ 代入各分量世代公式计算；归一化分母统一为 $(I_0 R^{h_{\text{gen}}})^2$；评估原点数 $M=5$。对九行实测记账各分量占总方差百分比计算 5 个原点的抽样标准误与极差（如 Delta $h=1$ 周未归因余项均值为 $92.58\% \pm 1.82\%$，极差 90.1\%--94.8\%；流感 4 周机制解释力均值为 $88.72\% \pm 2.45\%$，极差 85.3\%--91.5\%；RSV 两季 4 周未归因余项均值分别为 $95.41\% \pm 0.94\%$ 与 $95.08\% \pm 1.12\%$）；聚合顺序为先在各原点计算各项绝对误差均值再归一化记账；$\mathcal{E}_{\text{misspec}}$ 由记账恒等式残差定义闭合。$\text{CV}^2$ 与 $P$ 的绝对值严格由表~\ref{tab:horizons} 动力学参数集与代际公式精确算出。
\end{flushleft}
\end{table}
...
滚动样本外评估显示了清晰的技能衰减轨迹（图~\ref{fig:oos_skill}）：在近程步长 $h \le 2\text{--}4$ 周内，机制模型能够有效捕捉起步期的指数增长惯性，相对技能比显著低于 1（COVID-19 Delta 阶段 $h=1$ 周为 0.38，$h=2$ 周为 0.52；流感 $h=1$ 周为 0.42），展现出良好的预测效能；然而随着前瞻步长推进至 $h > 4$ 周，真实系统中由易感耗竭、行为自适应与未解释结构残差（占 72.6\%--95.4\%（短程 1--2 周为 72.6\%--94.2\%））累积带来的预测误差迅速扩张，技能比单调上升并在 $h \approx 3.8\text{--}4.6$ 周穿过 1.0 临界基准线（Delta 穿越点为 $4.5 \pm 0.4$ 周，Omicron 为 $3.8 \pm 0.3$ 周，流感为 $4.1 \pm 0.4$ 周，其中 $\pm$ 表示基于 2,000 次区块 Bootstrap 重抽样计算的标准误），随后预测误差劣化至朴素基线水平。这一滚动回测结果提供了清晰的实证映照：它表明在真实复杂系统中，实际有效预测时效受制于未建模外生扰动通常被压缩在 1--4 周内，而理论推导的 $h^*$（2.9--13.5 周）则确立了无外生扰动理想环境下的物理机制上限，两者共同构建了具有实践指导意义的双层分析框架。
...
\centering
\footnotesize
\caption{传染病理论机制可预测视界与实测业务可操作预测时效的双层对照}
\label{tab:two_tier_comparison}
\resizebox{0.88\textwidth}{!}{%
\begin{tabular}{lccc}
\toprule
\textbf{流行阶段 / 病原体} & \textbf{第一层：理论机制视界 $h^*_{\text{exact}}$} & \textbf{第二层：实测业务技能时效} & \textbf{比值（第一层 / 第二层）} \\
\midrule
COVID-19 Delta 阶段 & 13.5 周 (95\% CI 8.9--20.0 周) & $4.5 \pm 0.4$ 周 & 3.00 倍 \\
流感 2022--23 阶段 & 5.1 周 (95\% CI 3.3--10.2 周) & $4.1 \pm 0.4$ 周 & 1.24 倍 \\
COVID-19 Omicron 阶段 & 2.9 周 (95\% CI 1.8--6.0 周) & $3.8 \pm 0.3$ 周 & 0.76 倍 \\
\bottomrule
\end{tabular}%
}
\vspace{2pt}
\begin{flushleft}
\scriptsize \textbf{注：}第一层为 $\tau = 0.5$ 下全方差临界方程的数值精确根；第二层为滚动回测中点预测相对均方误差技能比穿过 1.0 的交叉点（$\pm$ 表示基于 2,000 次区块 Bootstrap 重抽样计算的标准误）。
\end{flushleft}
\end{table}

如表~\ref{tab:two_tier_comparison} 所示：对于 COVID-19 Delta 阶段，理论视界为 13.5 周、实测交叉点为 $4.5 \pm 0.4$ 周（比值 3.00 倍）；对于流感 2022--23 阶段，理论视界为 5.1 周、实测交叉点为 $4.1 \pm 0.4$ 周（比值 1.24 倍），二者展现了理论天花板高于现实操作时效的一致层级关系。对于 COVID-19 Omicron 阶段，理论视界为 2.9 周，而实测交叉点为 $3.8 \pm 0.3$ 周（比值 0.76 倍），呈现出实测交叉点略长于理论物理视界的表象。深入剖析表明，这一现象具有深厚的动力学根源：一方面，Omicron 属于高传播超分散株，早期爬坡推断标准误极高（$\delta R = 0.0762$），信噪比处于七阶段最低点（$R/\delta R = 15.27$），将第一层绝对误差容忍视界严重压缩；另一方面，第二层滚动评估衡量的是相对持续性基线的相对技能比（$\text{MSE}_{\text{model}} / \text{MSE}_{\text{persistence}}$），在 Omicron 极端陡峭的暴发拐点期，朴素持续性外推自身的预测均方误差发生灾难性劣化，导致模型预测即便绝对误差已触及容忍度，但在相对评分上仍能在较长时间内显著优于迅速崩溃的基线，从而被动推迟了技能比穿越 1.0 的时点。这一发现不仅消解了表观矛盾，反而深刻论证了构建双层分析框架的不可替代性：第一层度量系统绝对误差的物理容忍极限，第二层表征相对业务基线的操作优势区间，两者互为补充。需要说明的是，滚动评估采用点预测均方误差相对持续性基线口径（与 CDC FluSight 和 COVID-19 Hub 官方基线中位数预测完全对齐），前瞻步长设为 1--10 周旨在完整覆盖理论推导的 2.9--13.5 周量程。
...
从本文的数理模型出发：
\begin{equation}
\delta R_{\text{obs}}^2 \propto \frac{1}{\rho I_0}, \quad k_{\text{agg, obs}} \approx \rho k_{\text{micro}}.
\label{eq:sparse_sampling}
\end{equation}
式 (\ref{eq:sparse_sampling}) 的信息论根源在于监测序列漏报与抽样稀疏化所导致的 Fisher 信息量衰减。正如 Parag 等\citep{parag2022quantifying} 在关于含噪疫情曲线信息量量化工作中所证明的，监测覆盖率 $\rho$ 的降低以双重路径瓦解推断信噪比：一方面直接将观测到达过程的有效样本量压缩为 $\rho I_0$，导致参数估计方差逆向膨胀；另一方面稀疏抽样滤除了微观传播簇的高阶相关性，使得表观聚集度 $k_{\text{agg,obs}}$ 发生比例失真。这为后文所述的“逆向马太效应”提供了坚实的数理统计支撑。
...
本文针对传染病前瞻预测在近临界阶段频繁发生系统性失效的理论难题，在负二项分支过程动力学框架下推导并建立了前瞻预测相对均方误差的理论加性分解，给出了可预测视界领先阶解析闭式解 $h^*$ 与高阶精确数值根 $h^*_{\text{exact}}$。研究系统确立了超临界判别的 Cram\'er--Rao 独立样本量下限（单位信噪比基准 231 例与 80\% 统计功效基准 1,428 例）、有限宿主种群近临界扩散平稳态与临界慢化发散标度（$\mathcal{O}(N^{1/2})$）以及平稳自回归增长率下的方差累积律。机器学习基准实验表明，在生成机制完全正确设定的分支过程中，神经估计器（MLP）受有限样本统计效率制约，其经验方差显著高于理论下界（比值均值为 $6.69 \pm 0.42$，95\% 置信区间 6.27--7.11，基于 50 次独立随机初始化重复实验），而有效极大似然估计量在统计抽样噪声内与 Cram\'er--Rao 下界严格吻合（比值均值 1.000，95\% 蒙特卡洛置信区间全部覆盖 1.000），印证了统计学习估计量受底层信息论边界约束的客观规律。

在美国 CDC 真实监测数据（涵盖 COVID-19、季节性流感及 RSV 七大典型阶段，聚焦于早期指数爬坡相位）上的实证测算与四项预测误差记账核算显示，在文献校准代间隔与单流行季物理截断下，理论预测视界精确根点估计为 2.9--13.5 周（各流行阶段 95\% 置信区间并集为 1.8--20.0 周，RSV 截断前纯数学外推根为 43.9--70.1 周）。四项误差实测分解表明：在短程（1--2 周）尺度下，外生未归因结构性闭合余项占实测方差的 72.6\%--94.2\%（RSV 两季在 4 周步长下达 95.1\%--95.4\%）；而在多周长程累积中，已知动力学机制（特别是参数外推误差项）展现出几何级放大效应，在代间隔最短的流感 2022--23 暴发早期 4 周前瞻中机制解释力跃升至 88.7\%（未归因余项塌缩至 11.3\%）。伪实时滚动样本外预测验证进一步证实：在真实复杂系统中，由于外生干扰与结构性误设的累积，业务可操作预测时效通常被压缩至 1--4 周，随之衰减至朴素基线水平。本文揭示了宏观大样本聚合下人口随机性被大数定律平滑而视界由参数推断主导、微观小规模暴发下内在随机性构成刚性约束（由纯内在随机性视界上限 $h^*_{\text{stoch}}$ 刻画）的尺度依赖规律，提出并初步刻画了理论机制上限与业务可操作预测时效构成的双层分析框架。本文推导的可预测视界为量化事前预测能力提供了客观物理标尺，推动传染病监测预警从盲目追求远期外推转向基于数理极限的理性决策。
...
\vspace{0.4em}
\noindent\textbf{代码与材料可用性（Code Availability）：}本文涉及的所有数学公式推导复核脚本、高通量蒙特卡洛随机模拟程序、机器学习基准测试代码、高阶数值求根算法（$h^*_{\text{exact}}$ 40 节点 Gauss--Hermite 积分求解器）以及四项误差记账与逐周滚动回测评估脚本均已完全基于 Python 3.11 与开源科学计算栈（NumPy 1.26, SciPy 1.12, Pandas 2.2, LightGBM 4.3, Matplotlib 3.8）编写，并在标准计算环境（配置 AMD Ryzen 9 7950X / Intel Core i9-13900K，32GB 内存，单次全流程复现脚本运行耗时约 15 分钟）下完成全面复现检验。全部随机实验主种子均固定为 Seed 42，完整代码、参数配置文件与一键复现环境指南（\texttt{environment.yml}）已开源存放于匿名化公共代码仓库供同行审阅检验（访问链接：\url{https://anonymous.4open.science/r/epidemic-predictability-limits-2026}；如遇匿名托管平台访问限制，审稿专家可直接参阅评审专用离线代码数据包）。
