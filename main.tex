...
在逐代更新宏观负二项聚合模型下，设发病规模满足 $Z_t \mid \mathcal{F}_{t-1} \sim \text{NB}(\mu_t = R_t Z_{t-1}, k_{\text{agg}})$，对数更新满足 $\log Z_t = \log Z_{t-1} + \log R_t + b_t + \xi_t$（$b_t = \mathbb{E}[\log Z_t \mid \mathcal{F}_{t-1}] - \log \mu_t$ 为 Jensen 修正项）。由 Gamma--Poisson 混合表示，固定 $k_{\text{agg}}$、令 $\mu_t \to \infty$ 时，$Z_t/\mu_t \xrightarrow{d} \text{Gamma}(k_{\text{agg}}, k_{\text{agg}})$，相对波动不趋于零，对数方差严格收敛于 trigamma 函数 $\psi_1(k_{\text{agg}})$，且 $b_t \to \psi(k_{\text{agg}}) - \ln k_{\text{agg}}$。大离散度下由级数展开 $\psi_1(k) = 1/k + \mathcal{O}(k^{-2})$ 与 $b_t \approx -1/(2k) \to 0$ 导出领先阶 Delta 近似（实测各阶段 $\hat{k}_{\text{agg}}$ 中位数 11.2--56.6，高阶差额 $<5\%$）。叠加独立同分布环境扰动 $\log R_t = \log R + \eta_t$（$\eta_t \sim \mathcal{N}(0, v^2)$），在双重大样本条件（未灭绝 $Z_h > 0$、$\min_{t} \mu_t \gg k_{\text{agg}}$ 且 $k_{\text{agg}} \gg 1$）下，对数预测方差呈领先阶线性累积：
\begin{equation}
\text{Var}(\log Z_h \mid Z_h > 0) \sim h \left(v^2 + \frac{1}{k_{\text{agg}}}\right),
\end{equation}
固定 $k_{\text{agg}}$ 下其严格大均值极限速率为 $h(v^2 + \psi_1(k_{\text{agg}}))$。
\end{proposition}

% Proof sketch detailed in Appendix and Derivations Manual

作为命题~\ref{thm:t5} 环境噪声分量在时间自相关情形下的平行扩展，定理 5R 刻画传播增长率存在平稳 AR(1) 持续性漂移时的累积方差规律。命题~\ref{thm:t5} 针对 $\log Z_h$ 的方差，含环境噪声与抽样地板 $1/k_{\text{agg}}$（严格为 $\psi_1(k_{\text{agg}})$）；定理 5R 聚焦于增长率之和 $\sum_{j=1}^h r_{t+j}$，不含抽样项；完整方差在独立假设下将两者线性相加。
...
对于宏观更新模型，当各步发病充分大（$\min_{1 \le t \le h} \mu_t \gg k_{\text{agg}}$）且 $k_{\text{agg}} \gg 1$ 时，宏观发病对数方差满足领先阶近似：
\begin{equation}
\text{Var}(\log Z_h^{\text{agg}}) \approx \frac{h}{k_{\text{agg}}}.
\label{eq:pop_floor}
\end{equation}
固定 $k_{\text{agg}}$ 下 $\mu \to \infty$ 的严格渐近速率为 $h\,\psi_1(k_{\text{agg}})$；式~(\ref{eq:pop_floor}) 为 $k_{\text{agg}} \gg 1$ 下截断 trigamma 首项的一阶 Delta 近似，与发病规模解耦（实测中高阶差额仅 $1\%$--$5\%$）。
\end{corollary}
