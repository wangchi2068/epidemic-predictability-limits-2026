...
\end{equation}
该式表明机制视界随独立子代观测数 $C$ 以 $\sqrt{C}$ 的领先阶渐近标度增长，将数据充分性判据（定理~\ref{thm:fisher}）与视界映射（定理~\ref{thm:horizon}）桥接为同一组闭式，作为模型内的乐观外推标度参照而非任意有限样本预测器均满足的无条件严格上界。
\end{corollary}
...
在逐代独立更新的宏观负二项聚合模型下，设条件发病规模满足 $Z_t \mid \mathcal{F}_{t-1} \sim \text{NB}(\mu_t = R_t Z_{t-1}, k_{\text{agg}})$。其发病对数满足逐代更新方程：
\begin{equation}
\log Z_t = \log Z_{t-1} + \log R_t + b_t + \xi_t,
\end{equation}
其中 $b_t = \mathbb{E}[\log Z_t \mid \mathcal{F}_{t-1}] - \log \mu_t$ 为对数尺度下的 Jensen 修正项，$\xi_t \equiv \log Z_t - \mathbb{E}[\log Z_t \mid \mathcal{F}_{t-1}]$ 为鞅差创新。由负二项分布的 Gamma--Poisson 层次混合表示 $Z_t \mid \Lambda_t \sim \text{Poisson}(\Lambda_t)$（其中潜伏随机均值 $\Lambda_t \sim \text{Gamma}(k_{\text{agg}}, k_{\text{agg}}/\mu_t)$）：在固定离散参数 $k_{\text{agg}}$ 下，令发病期望规模 $\mu_t \to \infty$，泊松随机性相对消失，归一化发病变量弱收敛于标准化的 Gamma 分布：
\begin{equation}
\frac{Z_t}{\mu_t} \xrightarrow{d} G_t \sim \text{Gamma}(k_{\text{agg}}, k_{\text{agg}}), \qquad \mathbb{E}[G_t] = 1, \quad \text{Var}(G_t) = \frac{1}{k_{\text{agg}}}.
\end{equation}
因此，单步相对波动并不随发病绝对规模的增加而退化为零，对数变量的渐近条件方差严格收敛于三伽马（trigamma）函数 $\psi_1(k_{\text{agg}}) \equiv \psi'(k_{\text{agg}}) = \sum_{m=0}^\infty (k_{\text{agg}} + m)^{-2}$，而 Jensen 修正项严格收敛于 $b_t \to \psi(k_{\text{agg}}) - \ln k_{\text{agg}}$（$\psi$ 为 digamma 函数）。
...
对于跨种群聚合的宏观更新模型，当各步平均发病规模充分大（$\min_{1 \le t \le h} \mu_t \gg k_{\text{agg}}$）且宏观离散度满足 $k_{\text{agg}} \gg 1$ 时，记 $Z_h^{\text{agg}}$ 为第 $h$ 步宏观观测发病水平，其对数方差满足固定时间聚合尺度下的一阶近似累积律：
\begin{equation}
\text{Var}(\log Z_h^{\text{agg}}) \approx \frac{h}{k_{\text{agg}}},
\label{eq:pop_floor}
\end{equation}
在先固定有限提前期 $h$、再令各步均值充分大的极限下，其渐近累积速率在固定 $k_{\text{agg}}$ 下由三伽马函数给出：$\lim_{\min \mu_t \to \infty} \text{Var}(\log Z_h^{\text{agg}}) = h\,\psi_1(k_{\text{agg}})$。式~(\ref{eq:pop_floor}) 对应于大离散度 $k_{\text{agg}} \gg 1$ 下截断级数首项的一阶 Delta 近似，其显著特征是渐近累积速率与绝对发病规模 $\mu$ 解耦，单纯由宏观时序离散度 $k_{\text{agg}}$ 决定（在全美 7 个阶段跨州中位工作点上高阶差额约为 $0.9\%$--$4.5\%$）。
\end{corollary}
...
补充材料附表 S5 给出六个视界口径与实测时效的完整对照。插件式 NB2 模型内机制视界的州级中位数（2.2--7.1 周，多窗口范围覆盖 1.52--8.07 周）在全部七个阶段均高于同定义实测绝对视界（比值 1.1--4.0 倍）。但若代入极端个体级微观离散度（$\hat{k}=0.111$），低计数阶段视界可降至实测之下。因此主结论严格界定为：在固定离散度 NB2 宏观工作模型与中等个体离散度下，机制视界高于实测绝对时效；“宽松包络”应理解为该特定模型设定下的诊断性经验参考基准，而非现实预测系统的绝对物理上限。
...
\begin{enumerate}[leftmargin=*,itemsep=2.5pt,topsep=2.0pt]
...
命题~\ref{thm:t5} 宏观方差线性累积律 & 固定聚合离散度更新噪声累加 & 命题~\ref{thm:t5} & 鞅差对数创新求和，大均值与大 $k_{\text{agg}}$ 展开（大均值界含 $\psi_1$） & 手册第 5.1 节 式 (44)--(46) \\
定理 5R 平稳与单位根时变漂移 & 增长率时间自相关方差累积 & 定理 5R & 平稳自协方差级数求和；单位根下自然数平方和通项超线性发散 $\sim h^3/3$ & 手册第 5.2 节 式 (47)--(50) \\
推论~\ref{cor:pop_floor} 宏观抽样 $h/k_{\text{agg}}$ 律 & 跨代独立增量大样本噪声地板 & 式~(\ref{eq:pop_floor}) & 单代对数 Delta 展开，大均值与大 $k_{\text{agg}}$ 下为 $1/k_{\text{agg}}$（大均值界为 $\psi_1$） & 手册第 5.3 节 式 (51)--(52) \\
