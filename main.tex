...
在逐代独立更新的宏观负二项聚合模型下，设条件发病规模满足 $Z_t \mid \mathcal{F}_{t-1} \sim \text{NB}(\mu_t = R_t Z_{t-1}, k_{\text{agg}})$。需要指出，对任意有限 $\mu_t > 0$ 与 $k_{\text{agg}} > 0$，零计数概率 $\Pr(Z_t = 0 \mid \mathcal{F}_{t-1}) = (1 + \mu_t/k_{\text{agg}})^{-k_{\text{agg}}} > 0$ 恒正，直接作用的 $\log Z_t$ 在零点无定义且条件期望发散。为使条件期望与创新在有限样本下严格良定，本文采用正计数值截断对数协议（Truncated Logarithm Protocol）：定义 $L_t \equiv \log(Z_t \vee 1)$（在正计数路径 $Z_t \ge 1$ 上恒有 $L_t = \log Z_t$；在 $Z_t = 0$ 时为 $\log 1 = 0$）。则 $L_t$ 满足严格有限的逐代递推方程：
\begin{equation}
L_t = L_{t-1} + \log R_t + b_t + \xi_t,
\label{eq:truncated_log_recurrence}
\end{equation}
其中 $b_t \equiv \mathbb{E}[L_t \mid \mathcal{F}_{t-1}] - \log \mu_t$ 为良定的有限 Jensen 修正项，$\xi_t \equiv L_t - \mathbb{E}[L_t \mid \mathcal{F}_{t-1}]$ 为有限方差鞅差创新（$\mathbb{E}[\xi_t \mid \mathcal{F}_{t-1}] = 0$）。

由负二项分布的 Gamma--Poisson 层次混合表示 $Z_t \mid \Lambda_t \sim \text{Poisson}(\Lambda_t)$（其中随机均值 $\Lambda_t \sim \text{Gamma}(k_{\text{agg}}, k_{\text{agg}}/\mu_t)$）：在固定离散参数 $k_{\text{agg}}$ 下令条件期望规模 $\mu_t \to \infty$ 时，零事件概率以代数速率衰减 $\Pr(Z_t = 0) = \mathcal{O}(\mu_t^{-k_{\text{agg}}}) \to 0$，泊松相对随机性消失，归一化变量弱收敛于标准化的 Gamma 分布：
\begin{equation}
\frac{Z_t \vee 1}{\mu_t} \xrightarrow{d} G_t \sim \text{Gamma}(k_{\text{agg}}, k_{\text{agg}}), \qquad \mathbb{E}[G_t] = 1, \quad \text{Var}(G_t) = \frac{1}{k_{\text{agg}}}.
\end{equation}
由于 $G_t$ 的对数矩母函数在原点邻域解析且 $L_t - \log\mu_t$ 满足二阶一致可积性，由矩收敛定理，条件一阶与二阶矩收敛于对应 Gamma 对数矩：
\begin{equation}
\lim_{\mu_t \to \infty} b_t = \mathbb{E}[\log G_t] = \psi(k_{\text{agg}}) - \ln k_{\text{agg}}, \qquad \lim_{\mu_t \to \infty} \text{Var}(L_t \mid \mathcal{F}_{t-1}) = \text{Var}(\log G_t) = \psi_1(k_{\text{agg}}),
\end{equation}
其中 $\psi(k)$ 为 digamma 函数，$\psi_1(k) \equiv \psi'(k) = \sum_{m=0}^\infty (k + m)^{-2}$ 为三伽马（trigamma）函数。这表明单步相对方差并不随发病规模增加而消失，其大均值下界由三伽马函数 $\psi_1(k_{\text{agg}})$ 给出。

当进一步满足宏观大离散度条件（$k_{\text{agg}} \gg 1$）时，利用特殊函数的渐近级数展开：
\begin{equation}
...
\end{equation}
可得 $b_t \approx -1/(2k_{\text{agg}}) \to 0$ 成为高阶小量，单代抽样方差退化为一阶 Delta 近似值 $1/k_{\text{agg}}$。若对数再生数叠加独立同分布的环境扰动 $\log R_t = \log R + \eta_t$（$\eta_t \sim \mathcal{N}(0, v^2)$），在固定有限提前期 $h$、各步保持正计数且条件均值充分大（$\min_{1 \le t \le h} \mu_t \gg k_{\text{agg}}$）及 $k_{\text{agg}} \gg 1$ 条件下，未来第 $h$ 代发病对数指标的预测方差呈领先阶线性累积：
\begin{equation}
\text{Var}(L_h \mid Z_1 > 0, \dots, Z_h > 0) \sim h \left(v^2 + \frac{1}{k_{\text{agg}}}\right),
\end{equation}
而在固定 $k_{\text{agg}}$ 与固定有限 $h$ 下，其大均值极限累积速率由 $h(v^2 + \psi_1(k_{\text{agg}}))$ 给出。需要指出，上述结论针对所选定的截断对数协议（在正计数路径上与 $\log Z_t$ 完全重合），且按先固定有限提前期 $h$、预测路径各步保持正计数且条件均值充分大的极限理解；本文未证明该近似对增长的提前期 $h$ 一致有效。在各阶段跨州中位 $k_{\text{agg}} = 11.2\text{--}56.6$ 的工作点上，$1/k_{\text{agg}}$ 相对于 $\psi_1(k_{\text{agg}})$ 的领先阶差额 $\frac{\psi_1(k) - 1/k}{1/k} \approx \frac{1}{2k}$ 约为 0.9\%--4.5\%；这一范围描述的是阶段中位工作点，而非所有州—阶段单元。流感及 RSV 各阶段的对应值约为 1.3\%--4.5\%。因而 $1/k_{\text{agg}}$ 构成了阶段中位尺度上极佳且解析透明的工程领先阶近似。
\end{proposition}
...
对于跨种群聚合的宏观更新模型，在截断对数协议 $L_h = \log(Z_h^{\text{agg}} \vee 1)$ 下，当先固定有限提前期 $h$、预测路径各步均值充分大（$\min_{1 \le t \le h} \mu_t \gg k_{\text{agg}}$）且宏观离散度满足 $k_{\text{agg}} \gg 1$ 时，第 $h$ 步宏观发病对数方差满足固定时间聚合尺度下的一阶近似累积律：
\begin{equation}
...
命题~\ref{thm:t5} 宏观方差线性累积律 & 固定聚合离散度更新噪声累加 & 命题~\ref{thm:t5} & 截断对数鞅差创新求和，大均值与大 $k_{\text{agg}}$ 展开（大均值界含 $\psi_1$） & 手册第 5.1 节 式 (44)--(46) \\
...
\item \textbf{命题~\ref{thm:t5} 与推论~\ref{cor:pop_floor}}：截断对数 $L_t \equiv \log(Z_t \vee 1)$ 规避零点发散，确立鞅差创新有限性；大均值下 $(Z_t\vee 1)/\mu_t \xrightarrow{d} G_t$，二阶一致可积性确立一、二阶矩收敛至 $\psi(k)-\ln k$ 与 $\psi_1(k)$；大 $k_{\text{agg}}$ 级数展开截断给出 $1/k_{\text{agg}}$ 一阶累积近似。
\end{enumerate}
