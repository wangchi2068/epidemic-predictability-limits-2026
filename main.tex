...
-\dfrac{\ln\!\left(1 - \frac{I_0(R - 1)\tau^2}{1 + R/k}\right)}{\ln R}, & \text{CV}^2(1) \le \tau^2 < \text{CV}^2_\infty, \\[5pt]
\infty, & \tau^2 \ge \text{CV}^2_\infty, \\[3pt]
0, & \tau^2 < \text{CV}^2(1).
\end{cases}
...
\label{eq:pop_floor}
\end{equation}
其严格渐近大均值极限在固定 $k_{\text{agg}}$ 下由三伽马函数给出：$\lim_{\mu \to \infty} \text{Var}(\log Z_h^{\text{agg}}) = h\,\psi_1(k_{\text{agg}})$。式~(\ref{eq:pop_floor}) 对应于大离散度 $k_{\text{agg}} \gg 1$ 下截断级数首项的一阶 Delta 近似，其显著特征是渐近累积速率与绝对发病规模 $\mu$ 解耦，单纯由宏观时序离散度 $k_{\text{agg}}$ 决定（实证中高阶差额仅 $1\%$--$5\%$）。
\end{corollary}
...
州级结果与国家级结果方向一致、量级更短：相对持续性基线的州级中位穿越点为 Delta 3.7 周（IQR 2.9--5.0）、Omicron 1.4 周、JN.1 1.0 周、流感 2022--23 2.8 周、流感 2024--25 2.9 周、RSV 2024--25 1.3 周、RSV 2025--26 4.3 周，整体落在 1.0--4.3 周；国家级对应值分别为 3.52、1.82、1.00、5.34、4.10、1.57 与未穿越，落在 1.0--5.3 周（其中 RSV 2025--26 国家级在 8 周评估窗内未穿越）。相对局部线性基线的州级列给出的是 MSE 比自上方下穿 1.0 的插值步长（即在该下穿点附近本文模型追平局部线性基线；程序以首个插值交叉点为准，更远步长若再次波动不改变该初始追平点定义），七个阶段的中位下穿点为 1.0--1.2 周。需要指出，两个基线的穿越方向相反，具有不同的评估含义：相对持续性基线为预测技能丧失的上穿点（中位数 1.0--4.3 周），相对局部线性基线为模型追平基线的下穿点（约 1 周）。后者表明在多数州的最短步长上模型已不劣于局部线性基线，而非预测技能仅维持一周。因此，本文关于短业务时效的结论主要由持续性基线支撑；在短步长上，模型与局部线性基线表现相当。
...
命题~\ref{thm:t5} 宏观方差线性累积律 & 固定聚合离散度更新噪声累加 & 命题~\ref{thm:t5} & 鞅差对数创新求和，大计数与大 $k_{\text{agg}}$ 展开（严格界含 $\psi_1$） & 手册第 5.1 节 式 (44)--(46) \\
定理 5R 平稳与单位根时变漂移 & 增长率时间自相关方差累积 & 定理 5R & 平稳自协方差级数求和；单位根下自然数平方和通项超线性发散 $\sim h^3/3$ & 手册第 5.2 节 式 (47)--(50) \\
推论~\ref{cor:pop_floor} 宏观抽样 $h/k_{\text{agg}}$ 律 & 跨代独立增量大样本噪声地板 & 式~(\ref{eq:pop_floor}) & 单代对数 Delta 展开，大计数与大 $k_{\text{agg}}$ 下饱和于 $1/k_{\text{agg}}$（严格界为 $\psi_1$） & 手册第 5.3 节 式 (51)--(52) \\
