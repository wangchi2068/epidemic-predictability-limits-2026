...
def main():
    ...
    for key, rec in phases.items():
        ...
        for loc, v in rec["states"].items():
            ...
            row = {}
            R_w = v.get("R_week", float(np.exp(v["b_week"])))
            row["heuristic"] = solve(lambda hw: cv2_lemma(hw, k) + p_lognorm(hw, s_week) - TAU ** 2)
            row["macro"] = solve(lambda hw: float(macro_model.cv2_macro(hw, R_w, k, I0))
                                 + p_lognorm(hw, s_week) - TAU ** 2)
            row["macro_lin"] = solve(lambda hw: hw / k + p_lognorm(hw, s_week) - TAU ** 2)
