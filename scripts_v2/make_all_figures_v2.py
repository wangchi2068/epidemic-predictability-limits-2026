...
def gen_fig2():
    ...
    # Exact closed form from Theorem 5
    cv2_macro_exact = (R_m * (q**h_macro - R_m**h_macro)) / (I0_m * (R_m**(2*h_macro)) * (q - R_m)) + (1 + 1/k_agg)**h_macro - 1
    
    # Macro Monte Carlo simulation
    ...
    for t in range(len(h_macro)):
        ...
        Z_m[:, t + 1] = np.where(curr > 0, rng.negative_binomial(k_agg, np.clip(p_nb, 1e-6, 1 - 1e-6)), 0.0)
    ...
    fig.tight_layout()
    fig.savefig(FIGS / "fig2_cv_verify.png", dpi=300)
    plt.close(fig)
    print("[OK] fig2_cv_verify.png upgraded to publication-grade dual panel", flush=True)
