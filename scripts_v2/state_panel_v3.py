...
def solve_hstar(R, s, k, I0, tau=TAU):
    """Heuristic caliber: micro closed form evaluated at k_agg (generations)."""
...
def solve_hstar_macro(R_w, s_w, k, I0, tau=TAU):
    """Primary caliber: exact fixed-k_agg macro NB variance, weekly clock.

    R_w = exp(b_week) is the one-step (weekly) growth multiplier and s_w the
    weekly log-scale standard error, matching the clock on which k_agg was
    estimated; no generation-time conversion enters.
    """
    f = lambda h: float(macro_model.cv2_macro(h, R_w, k, I0)) \
        + p_lognorm(h, s_w) - tau ** 2
...
def analyze_segment(counts, mu_g):
    ...
    Two horizons are produced:
      h_week      : primary, macro-exact fixed-k_agg model on the weekly clock
      h_week_heur : micro closed form evaluated at k_agg (previous main caliber,
                    generation clock), retained as a scale-mapping sensitivity
    """
    ...
    delta_g = mu_g / 7.0
    R = float(np.exp(delta_g * b))      # generation-scale R (micro layer)
    s = float(delta_g * se_b)           # generation-scale log SE
    R_w = float(np.exp(b))              # weekly growth multiplier (macro model)
    s_w = float(se_b)                   # weekly log SE
    h_macro = solve_hstar_macro(R_w, s_w, k, I0)
    h_heur = solve_hstar(R, s, k, I0)
    out = {"R": R, "s": s, "R_week": R_w, "s_week": s_w,
           "k": k, "I0": I0, "window_total": float(counts.sum()),
           "b_week": b, "se_slope": se_b}
    out.update({
        "h_week": h_macro,
        "hs_week": h_macro * s_w if h_macro is not None else None,
        "h_week_heur": h_heur * delta_g if h_heur is not None else None,
        "h_gen": h_heur,
        "hs": h_heur * s if h_heur is not None else None,
    })
    return out
    ...
    for key, src, w0, w1, mu_g, disp in PHASES:
        ...
        for tau in (0.20, 0.35, 0.50, 0.70):
            ...
            for loc in sorted(STATE_FIPS):
                ...
                s_w = float(se_b)
                k = estimate_k(counts)
                h = solve_hstar_macro(R_w, s_w, k, float(counts.mean()), tau=tau)
                if h is not None:
                    hs.append(h)
            row[f"tau_{tau:.2f}"] = {
                "n": len(hs),
