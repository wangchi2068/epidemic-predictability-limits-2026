# 传染病传播动力学的可预测视界、极限机制与实证研究 (Epidemic Predictability Limits)

本目录为本论文全部**有效交付文件**的完整归档包。已彻底剥离历史草稿、冗余实验代码、中间缓存与编译日志。

> **关于稿件版本说明**：
> 1. 本次正式投稿以 **`paper_cn_journal_template/`（中文期刊模版主稿）** 与根目录下编译就绪的 **`流行病传播动力学可预测视界_最终稿.pdf`** 为唯一官方权威交付件。
> 2. 英文版属于早期预印本草稿，部分理论记号与推导口径尚未同步最新数理审改标准，本次投稿归档包中不纳入英文草稿，以避免版本口径冲突。

---

## 一、目录结构与文件导航

```text
.
├── 流行病传播动力学可预测视界_最终稿.pdf              # 最终排版生成的完整期刊论文 PDF（42 页，即开即读）
├── 流行病传播动力学的可预测视界、极限机制与实证研究_期刊模版版.docx  # 按期刊双栏模版排版的 Word 终稿
├── 流行病传播动力学的可预测视界、极限机制与实证研究_期刊模版版.pdf  # Word 版对应高清导出版（27 页）
├── Response_to_Reviewers_审稿逐条回复.md           # 针对各轮专家审稿意见的详尽逐条修改说明
├── 审稿意见.txt                                     # 原始终审专家审查报告与修改意见
├── README.md                                       # 本说明文件（项目架构与一键复现指南）
├── MANIFEST.md                                     # 全量有效交付文件与磁盘真实路径对照表
├── environment.yml                                 # Conda 运行环境配置文件
├── LICENSE                                         # 开源授权协议
├── paper_cn_journal_template/                      # 中文期刊模版完整源码包
│   ├── main.tex                                    # 最终修订版 LaTeX 主文档（0 错误、0 溢出）
│   ├── main.pdf                                    # Tectonic / XeLaTeX 编译产物 (1.35 MiB)
│   ├── references.bib                              # 净化后的参考文献库（57 篇，0 未引项）
│   ├── fig1_framework.png                          # 理论原理图高清矢量渲染图
│   ├── Response_to_Reviewers.md                    # 审稿意见逐条回复
│   ├── Response_to_Reviewers_round16.md            # 第 16 轮审稿回复对照
│   ├── 审稿意见.txt                                 # 原始审稿意见备份
│   ├── 流行病传播动力学的可预测视界、极限机制与实证研究_期刊模版版.docx
│   └── 流行病传播动力学的可预测视界、极限机制与实证研究_期刊模版版.pdf
├── derivations/                                    # 数学定理严格推导与证明手册
│   ├── derivations_manual.tex                      # 逐行数学证明源码
│   └── derivations_manual.pdf                      # 推导手册编译 PDF
├── presentation/                                   # 成果报告与展示材料
│   ├── epidemic_predictability_masterclass.pptx    # Masterclass 汇报幻灯片
│   └── index.html                                  # 交互式结果全景展示网页
├── data/                                           # 实证研究原始与清洗后核心数据集
│   ├── micro/                                      # 个体传播链微观数据
│   │   ├── guinea_ebola_faye2015/                  # 几内亚埃博拉传播网络数据
│   │   ├── hong_kong_adam2020/                     # 香港 COVID-19 接触追踪集群数据
│   │   ├── lloyd_smith_2005/                       # 超级传播文献基准
│   │   └── micro_branching_fit_results.json        # 微观似然拟合参数结果
│   ├── panels/                                     # 51 个州级面板数据 (Delta, Omicron, Flu, RSV)
│   │   ├── covid_weekly_hospitalizations.csv.gz    # COVID-19 州级周度住院序列
│   │   ├── flu_weekly_hospitalizations.csv.gz      # 流感州级周度住院序列
│   │   └── rsv_weekly_hospitalizations.csv.gz      # RSV 州级周度住院序列
│   └── hub/                                        # COVID-19 Forecast Hub 官方顶级集成模型预测与评估
│       ├── hub_forecasts_with_baseline.csv.gz      # 集成预测与持续性基线对比数据
│       └── verify_forecast_hub.json                # 预测枢纽评估结果摘要
├── scripts_v2/                                     # 生产级可复现分析、制表与检验代码集
│   ├── check_consistency.py                       # 8 大类核心数理与实证断言自动化一致性检验脚本
│   ├── emit_v3.py                                  # 表格 LaTeX 片段全自动生成脚本
│   ├── state_panel_v3.py                           # 州级面板分析与滚动回测主脚本
│   ├── make_all.py                                 # 全流程一键复现主入口
│   └── ...                                         # 各项拟合与绘图脚本
├── reports/                                        # 基础理论数值模拟输出与图件
│   ├── *.json                                      # 数值模拟输出结果
│   └── figures_v2/                                 # 理论推导与下界验证图件 (图 2, 图 3, 图 4)
├── reports_v3/                                     # 经验实证分析产物与图件
│   ├── *.json                                      # 州级视界、四项误差记账与预测枢纽实证结果
│   └── figures/                                    # 实证结果高清图件 (微观拟合, 州级视界, Hub 技能衰减)
└── tables_v3/                                      # 导出的独立 LaTeX 表格源码片段
    ├── table7_micro.tex                            # 正文表 2: 微观似然拟合与 CRB
    ├── table2_state.tex                            # 正文表 3: 51 州理论视界空间分布
    ├── table4_budget.tex                           # 正文表 4: 四项描述性误差记账
    ├── table3_rolling.tex                          # 正文表 5: 伪实时业务时效双层对照
    ├── table5_hub.tex                              # 正文表 6: 预测枢纽集成预测技能衰减
    └── table6_tiers.tex                            # 正文表 7: 分级预警风控推荐场景
```

---

## 二、快速复现与一键检验指南

### 1. 执行全量一致性检验套件
在本项目根目录下运行：
```bash
python scripts_v2/check_consistency.py
```
该脚本将对全文字面值、表格数值、图件完整性、误差记账恒等关系、CRB 自助比值等 **8 大类核心断言** 实施自动化交叉核验。

### 2. 重新编译 LaTeX 论文
进入中文期刊模版目录：
```bash
cd paper_cn_journal_template
tectonic -X compile --keep-logs main.tex
```
也可使用标准 XeLaTeX 编译：
```bash
xelatex main.tex
bibtex main.aux
xelatex main.tex
xelatex main.tex
```
当前主稿编译指标：**0 Errors, 0 Overfull hboxes, 0 Undefined References**。
